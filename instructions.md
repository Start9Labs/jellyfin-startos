...
- Read-only access to media stored on **FileBrowser Quantum** and/or **Nextcloud**, mounted at `/mnt/filebrowser` and `/mnt/nextcloud` inside Jellyfin. Jellyfin does not read arbitrary filesystem paths on StartOS — your media lives in one of those two services.

## Getting set up

Jellyfin posts a critical **Select Media Sources** task after install. You can't start the service until it's done.

1. Install **FileBrowser Quantum**, **Nextcloud**, or both, and upload your media to them.
...
- **Select Media Sources** — change which of FileBrowser Quantum and Nextcloud Jellyfin reads from. Changing sources updates Jellyfin's dependencies; if you remove a source, also remove or repoint any Jellyfin libraries that referenced its mount point.
...
- **No arbitrary media paths.** Jellyfin can only read media from FileBrowser Quantum and/or Nextcloud on StartOS — not from arbitrary host directories.
